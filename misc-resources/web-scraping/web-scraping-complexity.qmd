...
- Web scraping is the use of programming to extract text or data from a website
- It is generally used to automate tasks that would take too long to be feasible manually.
...
- Downloading hundreds of CSV files from the Centers for Medicare and Medicaid Services that all required clicking different dropdownsfrom a menu of options
- (Erika's examples here)

## What are some drawbacks of web scraping?
- Not all sites can be legally or responsible scraped
...
1. How many different websites are involved in the web scraping analysis?
2. Does the website have dynamic content or only static content?
3. Is it easy to identify the info we want once we reach the desired webpage?
...
- If the layouts of the sites are different, difficulty vastly increases
  - **Rule of thumb**: Think of this as a unique web scraping task for each uniquely arranged website
...
- For a static page like a [Wikipedia article](https://en.wikipedia.org/wiki/Urban_Institute), packages like `BeautifulSoup` can grab HTML text without too much complexity by parsing HTML tags
- For pages with dynamic content like clickable buttons or dropdown menus, the `Selenium` package is needed and the complexity goes up
  - **Rule of thumb**: Would a human user need to take any actions (besides scrolling up or down) to navigate to the desired info, or can it be identified on the webpage?
...
- Does the webpage suffer from slow load times?
- Are there any site/rate limits or responsible web scraping considerations?
